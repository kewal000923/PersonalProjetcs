...
class AdminController extends Controller {
	...
	public function ads(){
		$data = Ads::orderBy('id')->get();

		return view('admin.ads')->withData($data);
	}
	...
	public function stroredAds(Request $request){
		$temp            = 'ads-image/temp/'; // Temp
		$path            = 'ads-image/'; // Path General
		  ...
		  $sql->url        = strtolower($request->slug);
		  $sql->thumbnail = $thumbnail;
		  $sql->mode        = $request->mode;
		  $sql->adsshow	= $request->adsShow;
		  $sql->save();
  
		  \Session::flash('success_message', trans('admin.success_add_category'));
  
		  return redirect('panel/admin/ads');
	}
	public function editAds($id) {

		$ads = Ads::find( $id );

		return view('admin.edit-ads')->with('ads',$ads);

	}
	public function updateAds( Request $request ) {


		$categories  = Ads::find( $request->id );
		$temp            = 'ads-image/temp/'; // Temp
		$path            = 'ads-image/'; // Path General
		...
		$type_mime_shot   = $request->file('thumbnail')->getMimeType();
		$sizeFile                 = $request->file('thumbnail')->getSize();
		$thumbnail              = $request->url.'-'.str_random(32).'.'.$extension;
		...
		$categories->url        = strtolower($request->slug);
		$categories->thumbnail  = $thumbnail;
		$categories->mode        = $request->mode;
		$categories->adsshow        = $request->adsShow;
		$categories->save();
	...
	public function deleteAds($id){
		...
		$categories        = Ads::find( $id );
		$thumbnail          = 'ads-image/'.$categories->thumbnail; // Path General
		...
		if( !isset($categories) || $categories->id == 1 ) {
			...
			\Session::flash('success_message', trans('admin.success_remove_ads'));
			return redirect('panel/admin/ads');
		}
	}
	//Ads End
	...
	public function themeStore(Request $request)
	{
		...
		$pathCategory = 'img-category/'; // Path Category
